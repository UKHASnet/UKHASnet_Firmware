...
static uint32_t count = 0, data_interval, j;
...
uint8_t getRandBetween(uint8_t lower, uint8_t upper);
...
int16_t gen_data(char *buf)
{
    ...
    temp = -100;
    uint8_t timeout=0;
    while((temp<-10 || temp>70) && timeout<10)
    {
        temp = rf69_readTemp();
        timeout++;
    }
...
int main(void)
{
    ...
    while(1)
    {
        if(!zombie_mode) {
            for(uint8_t i=0; i<255; i++) {
              ...
              if (rf69_receive(buf, &len, &lastrssi))
              {
                ...
                if (buf[0] > '0' && end_bracket != -1 && strstr((const char *)&buf[start_bracket], NODE_ID) == NULL) {
                  ...
                  uint8_t delayValue = getRandBetween(5u, 80u);
            ...
            /* Zombie Mode */
            _delay_ms(8000u); // TODO: actually go low power
        }
...
uint8_t getRandBetween(uint8_t lower, uint8_t upper){
    return (uint8_t)(((double)rand() / ((double)RAND_MAX + 1) * (upper-lower))+lower);
}
